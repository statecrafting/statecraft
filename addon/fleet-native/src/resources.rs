//! Pure builders for the EnRaHiTu placement shape (spec 006 §3). Each returns a
//! typed `k8s-openapi` object; no I/O. `kube_ops` applies them to a cluster.
//!
//! Shapes are informed by deployd-api-rs' `acme-vue-encore` chart (spec 006 §1)
...
/// Baseline per-namespace NetworkPolicies (spec 006 §3): default-deny, allow the
...
/// + clean-shutdown consistency), then back up after (spec 006 §3).
pub fn backup_job(
    spec_name: &str,
    namespace: &str,
    target: &BackupTarget,
